...
if Place == 0x2002 and Events(0x01,Null,0x01) then --Station of Serenity Weapons
	...
	BitOr(Save+0x1CD5,0x08)  --TT_SCENARIO_1_END (Dusk Reward)
	...
	WriteShort(Save+0x1F88,0x8288) --Antechamber Barrier
	...
	--Start at 2nd Visits
	WriteByte(Save+0x1D9F,4) --Land of Dragons
	WriteByte(Save+0x1D3F,7) --Beast's Castle
	WriteByte(Save+0x1E5F,7) --Halloween Town
	WriteByte(Save+0x1D7F,6) --Agrabah
...
if ReadByte(Save+0x1D9F) > 4 then
	if Place == 0x0208 then --Checkpoint -> Encampment
		Spawn('Short',0x03,0x024,0x0002)
...
elseif ReadByte(Save+0x1D3F) == 7 and ReadByte(Save+0x35B3) > 0 then --2nd Visit
	...
	WriteShort(Save+0x0790,0x00) --Entrance Hall MAP (Visibility)
...
elseif Place == 0x0605 and Events(Null,Null,0x0B) then --Stay With Me
	...
	WriteShort(Save+0x0790,0x01) --Entrance Hall MAP (Visibility)
	WriteShort(Save+0x0794,0x01) --Entrance Hall EVT
end
--Block 1st Visit Areas
...
elseif Place == 0x0307 and Events(Null,Null,0x03) then --Come Back Soon
	...
	WriteShort(Save+0x0A9E,0x02) --The Peddler's Shop (Poor) BTL
elseif ReadByte(Save+0x1D7F) == 6 and ReadByte(Save+0x35C0) > 0 then --2nd Visit
	WriteByte(Save+0x1D7F,7)
	WriteShort(Save+0x0A94,0x00) --Agrabah EVT
...
if ReadShort(Save+0x0AEC) == 0x01 and ReadByte(Save+0x1D7F) < 11 then --Agrabah Not Cleared
...
elseif Place == 0x2204 and Events(0x7D,0x7D,0x7D) then --Zexion Defeated
	...
	WriteShort(Save+0x0922,0x01) --Underworld Entrance MAP
	WriteShort(Save+0x0924,0x16) --Underworld Entrance BTL
	WriteShort(Save+0x0936,0x16) --Hades' Chamber BTL
	WriteShort(Save+0x094E,0x16) --Cave of the Dead: Inner Chamber BTL
end
--Block 1st Visit Areas
...
if ReadShort(Save+0x094E) == 0x01 and ReadByte(Save+0x1D6F) < 11 then --Olympus Coliseum Not Cleared
	WriteShort(Save+0x094E,0x00) --Cave of the Dead: Inner Chamber BTL
end
end
...
elseif Place == 0x2104 and Events(0x81,0x81,0x81) then --Larxene Defeated
	...
	WriteShort(Save+0x1990,0x00) --Pit Cell MAP (Locked In)
	WriteShort(Save+0x1996,0x02) --Canyon MAP (With Cubes Antenna)
	WriteShort(Save+0x199A,0x01) --Canyon EVT
	WriteShort(Save+0x19C2,0x00) --Central Computer Mesa BTL
end
